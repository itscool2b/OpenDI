...
Computes the gradient of the loss with respect to the second matrix in a matrix multiplication.
...
Where dout is the m×p upstream gradient. The result is an n×p matrix matching the shape of B.

## Parameters

- `arena`: Arena allocator for memory
- `a`: Pointer to the first matrix from the forward pass (m×n matrix)
...
A pointer to memory in the arena containing the n×p gradient matrix.
...
double *grad = matmul_backward_b(arena, a, dout, 2, 2, 2);
// grad = A^T @ dout = {1, 3, 2, 4}
...
matmul_backward_a(3), matmul(3), mattranspose(3)
